...
void MultiPowerMode_::drawGraph()
{
  ...
  for (uint8_t i = 0; i < GRAPH_DATA_ITEMS_SM; i++) {
    ...
    uint16_t maxPower = 10 * (MaxPowerWatts.get() / 4);
    uint32_t height = ((uint32_t)graphAData[idx] * MATRIX_SIZE_Y) / maxPower ;
    if (height > MATRIX_SIZE_Y)
    {
      height = MATRIX_SIZE_Y;
    }
    if (graphAData[idx] > 0) {
      Matrix.line(MATRIX_GREEN, 64 + GRAPH_DATA_ITEMS_SM - i - 1, MATRIX_SIZE_Y - 1, 64 + GRAPH_DATA_ITEMS_SM - i - 1, height < MATRIX_SIZE_Y ? MATRIX_SIZE_Y - 1 - height : 0);
    }
    height = ((uint32_t)graphBData[idx] * MATRIX_SIZE_Y) / maxPower ;
    if (graphBData[idx] > 0) {
      Matrix.line(MATRIX_RED, 96 + GRAPH_DATA_ITEMS_SM - i - 1, MATRIX_SIZE_Y - 1, 96 + GRAPH_DATA_ITEMS_SM - i - 1, height < MATRIX_SIZE_Y ? MATRIX_SIZE_Y - 1 - height : 0);
    }
    height = ((uint32_t)graphCData[idx] * MATRIX_SIZE_Y) / maxPower ;
    if (graphCData[idx] > 0) {
    ...
    height = ((uint32_t)graphDData[idx] * MATRIX_SIZE_Y) / maxPower ;
    if (graphDData[idx] > 0) {
      Matrix.line(MATRIX_GREEN, 32 + GRAPH_DATA_ITEMS_SM - i - 1, MATRIX_SIZE_Y - 1, 32 + GRAPH_DATA_ITEMS_SM - i - 1, height < MATRIX_SIZE_Y ? MATRIX_SIZE_Y - 1 - height : 0);
    }
  }
}
...
void MultiPowerMode_::drawValue()
{
  ...
  Matrix.rtext(MATRIX_RED, 95, 15, String(lastAValue / 10));
  Matrix.rtext(MATRIX_GREEN, 127, 15, String(lastBValue / 10));
  Matrix.rtext(MATRIX_GREEN, 31, 15, String(lastCValue / 10));
  Matrix.rtext(MATRIX_RED, 63, 15, String(lastDValue / 10));
